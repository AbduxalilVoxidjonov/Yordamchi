...
// PdfSharp also defines a PageRotation/PageSize pair; alias them so Yordamchi.Models wins.
...
public sealed class PdfService : IPdfService
{
        ...
        CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            return await Task.Run(async () =>
            {
                ...
                var sizes = PDFtoImage.Conversion.GetPageSizes(pdfBytes, password);
                if (sizes.Count == 0)
                    throw new PdfServiceException(PdfErrorKind.EmptySelection, $"'{Path.GetFileName(filePath)}' faylida sahifa yo'q.", filePath);
                    ...
                    .ConfigureAwait(false))
                {
                    ...
                    index++;
                    progress?.Report(new PdfProgress(index, sizes.Count, $"{index}/{sizes.Count} sahifa chizilmoqda"));
                }
        ...
        CancellationToken cancellationToken = default)
    {
        try
        {
            if (inputFiles is null || inputFiles.Count == 0)
                throw new PdfServiceException(PdfErrorKind.EmptySelection, "Birlashtirish uchun fayl tanlanmadi.", outputPath);
        ...
        CancellationToken cancellationToken = default)
    {
        if (keepPageIndicesInOrder is null || keepPageIndicesInOrder.Count == 0)
            throw new PdfServiceException(PdfErrorKind.EmptySelection, "Natijada birorta ham sahifa qolmaydi.", outputPath);
        ...
        CancellationToken cancellationToken = default)
    {
        try
        {
            if (imagePaths is null || imagePaths.Count == 0)
                throw new PdfServiceException(PdfErrorKind.EmptySelection, "Rasm tanlanmadi.", outputPath);
    ...
    public async Task<int> GetPageCountAsync(string filePath, string? password = null, CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            // Conversion ning satrli overloadlari yo'l emas, base64 kutadi — doim baytlarni beramiz.
            return await Task.Run(() => PDFtoImage.Conversion.GetPageCount(pdfBytes, password), cancellationToken).ConfigureAwait(false);
        }
        catch (OperationCanceledException)
        {
            throw;
        ...
        CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            {
                var pageCount = PDFtoImage.Conversion.GetPageCount(pdfBytes, password);
                if (pageIndex < 0 || pageIndex >= pageCount)
                    throw new PdfServiceException(
                        PdfErrorKind.PageIndexOutOfRange,
                        $"'{Path.GetFileName(filePath)}' faylida {pageIndex + 1}-sahifa yo'q (jami {pageCount} ta).",
                        filePath);

                cancellationToken.ThrowIfCancellationRequested();
                using var bitmap = PDFtoImage.Conversion.ToImage(pdfBytes, pageIndex, password, CreateRenderOptions(width));
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (output.PageCount == 0)
            throw new PdfServiceException(PdfErrorKind.EmptySelection, "Tanlangan fayllarda sahifa yo'q.", outputPath);
        ...
        try
        {
            ...
            foreach (var edit in pages)
            {
                ...
                if (edit.SourcePageIndex < 0 || edit.SourcePageIndex >= document.PageCount)
                    throw new PdfServiceException(
                        PdfErrorKind.PageIndexOutOfRange,
                        $"'{Path.GetFileName(edit.SourceFilePath)}' faylida {edit.SourcePageIndex + 1}-sahifa yo'q (jami {document.PageCount} ta).",
                        edit.SourceFilePath);
            }
            ...
            foreach (var edit in pages)
            {
                ...
                progress?.Report(new PdfProgress(done, pages.Count, $"{done}/{pages.Count} sahifa yozilmoqda"));
            }
    ...
    private static void AddImagePage(PdfDocument document, string imagePath, ImageToPdfOptions options)
    {
        ...
        catch (Exception ex) when (ex is not OperationCanceledException and not OutOfMemoryException)
        {
            ...
            throw new PdfServiceException(
                ...
                $"'{Path.GetFileName(imagePath)}' rasm sifatida o'qilmadi.",
                imagePath,
                ex);
        }
    ...
    private static MemoryStream ReencodeForPdf(string imagePath, ImageToPdfOptions options)
    {
        ...
        try
        {
            ...
            using var data = bitmap.Encode(format, quality)
                ?? throw new NotSupportedException($"'{Path.GetFileName(imagePath)}' qayta kodlanmadi.");
    ...
    private static void SaveAtomically(PdfDocument document, string outputPath)
    {
        ...
        catch (Exception ex) when (ex is UnauthorizedAccessException or IOException)
        {
            throw new PdfServiceException(
                ...
                $"'{Path.GetFileName(outputPath)}' fayliga yozib bo'lmadi. U boshqa dasturda ochiq bo'lishi mumkin.",
                outputPath,
                ex);
        }
        finally
    ...
    private static void ValidateOutputPath(string? outputPath)
    {
        ...
        {
            throw new PdfServiceException(PdfErrorKind.OutputNotWritable, $"'{outputPath}' — yaroqli fayl yo'li emas.", outputPath, ex);
        }
    ...
    private static PdfServiceException Wrap(Exception exception, string? filePath, PdfErrorKind fallback = PdfErrorKind.Unknown)
    {
        ...
        if (exception is UnauthorizedAccessException or IOException)
            return new PdfServiceException(PdfErrorKind.OutputNotWritable, $"{name} fayliga yozib bo'lmadi. U boshqa dasturda ochiq bo'lishi mumkin.", filePath, exception);

        if (fallback == PdfErrorKind.UnsupportedImage)
            return new PdfServiceException(PdfErrorKind.UnsupportedImage, $"{name} — qo'llab-quvvatlanmaydigan rasm formati.", filePath, exception);

        if (fallback == PdfErrorKind.CorruptedDocument)
            return new PdfServiceException(PdfErrorKind.CorruptedDocument, $"{name} shikastlangan yoki PDF emas.", filePath, exception);

        return new PdfServiceException(PdfErrorKind.Unknown, $"{name} ni qayta ishlab bo'lmadi.", filePath, exception);
    }
